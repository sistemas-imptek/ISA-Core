...
//	List<Object[]> findOnlyProductById(@Param("idP") Integer idP);
	...
	@Query(value = "select p.product_id, p.product_sap_code, p.product_name, p.product_description, p.product_type, pl.propl_id, pl.propl_name, prop.property_type, pl.propl_periodicity, pl.propl_norm, prop.property_min, prop.property_max, prop.property_unit, prop.property_view, prop.property_view_hcc  from product p  inner join property prop on p.product_id = prop.product_id inner join propertylist pl on prop.propl_id = pl.propl_id where p.product_id = :idP and pl.propl_periodicity = :period", nativeQuery = true)
